...
public class MapEditor : IDisposable {
    ...
    public string NotePasteBehavior {
        get {
            return uiAdapter.GetUserSetting(UserSettingsKey.NotePasteBehavior);
        }
    }
    ...
    public MapEditor(IMapEditorUiAdapter uiAdapter, string folderPath, bool makeNewMap) {
    ...
    private void SwapDifficulties(int i, int j) {
        ...
        uiAdapter.UpdateDifficultyButtons();
    }
    public void AddBPMChange(BPMChange b, bool redraw = true) {
        currentMapDifficulty?.MarkDirty();
        currentMapDifficulty?.bpmChanges.Add(b);
        if (redraw) {
            uiAdapter.DrawEditorGrid(false);
        }
        uiAdapter.RefreshBPMChanges();
    }
    ...
    public void UpdateNotes(IEnumerable<Note> newNotes, IEnumerable<Note> oldNotes, bool updateHistory = true) {
        ...
        uiAdapter.UndrawNotes(undrawNotes);
        uiAdapter.UndrawNavNotes(undrawNotes);
    ...
    public void RemoveNotes(IEnumerable<Note> notes, bool updateHistory = true) {
        ...
        // undraw the added notes
        uiAdapter.UndrawNotes(noteList);
        uiAdapter.UndrawNavNotes(noteList);
    ...
    public void SelectNotes(IEnumerable<Note> notes, bool updateMapStats = true) {
        ...
        uiAdapter.HighlightNotes(selectNotes);
        uiAdapter.HighlightNavNotes(selectNotes);
        if (updateMapStats) {
            RecalculateMapStats();
        }
    }
    public void SelectAllNotes() {
        ...
        uiAdapter.HighlightAllNotes();
        uiAdapter.HighlightAllNavNotes();
        RecalculateMapStats();
    }
    ...
    public void UnselectNote(Note n, bool updateMapStats = true) {
        ...
        uiAdapter.UnhighlightNotes([n]);
        uiAdapter.UnhighlightNavNotes([n]);
    ...
    public void UnselectAllNotes(bool updateMapStats = true) {
        ...
        uiAdapter.UnhighlightAllNotes();
        uiAdapter.UnhighlightAllNavNotes();
    ...
    public void CopySelection() {
        ...
        try {
            ...
            uiAdapter.SetClipboardText(RuneStringSerializer.Serialize(runeStringData, RagnaRuneString.Version.VERSION_1));
        } catch (Exception ex) {
    ...
    public void PasteClipboard(double beatOffset, int? colStart) {
        ...
        var clipboardText = uiAdapter.GetClipboardText();
    ...
    public void RetimeNotesAndMarkers(double newBPM, double oldBPM) {
        if (currentMapDifficulty == null) {
            return;
        }
        currentMapDifficulty.MarkDirty();
    ...
    public void SelectNotesInBookmark(Bookmark b) {
        if (currentMapDifficulty == null) {
            return;
        }
    ...
    public void SelectNotesInBPMChange(BPMChange bpmChange) {
        if (currentMapDifficulty == null) {
            return;
        }
    ...
    internal void RecalculateMapStats() {
        ...
        uiAdapter.SetMapStats(currentDifficultyStats);
    }
}
