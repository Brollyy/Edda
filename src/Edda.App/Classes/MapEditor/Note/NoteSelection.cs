...
namespace Edda.Classes.MapEditorNS.NoteNS {
    ...
    public class NoteSelection {
        ...
        public NoteSelection(MapEditor editor) {
            var mapDifficulty = editor.currentMapDifficulty ?? throw new InvalidOperationException("Cannot copy note selection when no map difficulty is selected.");
            notes = mapDifficulty.selectedNotes;
            bpmChanges = new(
                mapDifficulty.bpmChanges
        ...
        private IEnumerable<Note> GetPasteNotesAlignToNoteBPM(MapEditor editor, double beatOffset, int? colStart) {
            var editorDifficulty = editor.currentMapDifficulty;
            if (editorDifficulty == null) {
                yield break;
            }
            ...
            foreach (var note in notes) {
                ...
                while (Helper.DoubleApproxGreater(note.beat, noteSelectionBeat)) {
                    ...
                    if (!noteSelectionMarchEditorBeatChange.Equals(editorBeatChange)) {
                        ...
                        var nextEditorBeatChange = editorDifficulty.bpmChanges.GetViewBetween(editorBeatChange, noteSelectionMarchEditorBeatChange).SkipWhile(bpmChange => bpmChange.Equals(editorBeatChange)).FirstOrDefault() ?? editorBeatChange;
