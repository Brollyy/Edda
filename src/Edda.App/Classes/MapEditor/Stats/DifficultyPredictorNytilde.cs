    ...
    public class DifficultyPredictorNytilde : IDifficultyPredictor {
        ...
        public float? PredictDifficulty(IReadOnlyCollection<Note> notes, double globalBPM, double songDuration) {
            var sortedNotes = notes
                ...
                .ThenBy(note => note.col)
                .ToList();
            var timeSeries = sortedNotes.Select(note => 60 / globalBPM * note.beat).ToList();
            ...
            var melchiorDiffScore = GetMelchiorDifficultyScore(sortedNotes, globalBPM);
            ...
            var modelPath = CheckParameterRanges(noteDensity, averageTimeDifference, countNoteDensityPerWindow, highLocalNoteDensity, typicalTimeDifference)
                ? GetBundledModelPath("Edda-MLDP_Nytilde.onnx")
                : GetBundledModelPath("Edda-MLDP_Nytilde_Fallback.onnx");

            return EvaluateModel(modelPath, (float)noteDensity, (float)averageTimeDifference, (float)melchiorDiffScore, countNoteDensityPerWindow, (float)highLocalNoteDensity, (float)typicalTimeDifference);
        }

        private static float EvaluateModel(string modelPath, float noteDensity, float averageTimeDifference, float melchiorDiffScore, float countNoteDensityPerWindow, float peakNoteDensity, float typicalTimeDifference) {
        ...
        private static string GetBundledModelPath(string fileName) {
            var path = Path.Combine(AppContext.BaseDirectory, Program.ResourcesPath, fileName);
            if (File.Exists(path)) {
                return path;
            }
            ...
            throw new FileNotFoundException($"Couldn't find difficulty model file {fileName} in {AppContext.BaseDirectory}");
        }
        ...
        private static double GetMelchiorDifficultyScore(IReadOnlyList<Note> notes, double globalBPM, double timeThreshold = 0.02) {
