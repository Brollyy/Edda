...
internal static class AvaloniaWindowTestHarness {
    ...
    internal static void RunOpenedFixtureMapTest(Action<AvaloniaUIDriver, string> testBody, Action<AvaloniaUIDriver>? configureDriver = null) {
    ...
    internal static string LaunchAndOpenFixtureMap(AvaloniaUIDriver driver) {
    ...
    internal static string CreateTempOutputFolder(string tag) {
        var outputPath = Path.Combine(Path.GetTempPath(), "Edda-AvaloniaWindowTests", tag, Guid.NewGuid().ToString("N"));
    ...
    internal static void LaunchAndOpenMap(AvaloniaUIDriver driver, string mapFolder) {
        driver.Launch();
        driver.WaitForIdle();
    ...
    static string BuildStepManiaSimfile(string title, string songFileName) {
        return
            $"#TITLE:{title};\n" +
            "#SUBTITLE:;\n" +
