...
var info = Directory.GetFiles(work, "*.dat", SearchOption.AllDirectories)
    .FirstOrDefault(p => string.Equals(Path.GetFileName(p), "info.dat", StringComparison.OrdinalIgnoreCase));
if (info == null) throw new Exception("Could not find info.dat");
...
double songDuration =
    infoJson["_songDuration"]?.Value<double>() ??
    infoJson["songDuration"]?.Value<double>() ??
    infoJson["_songApproximativeDuration"]?.Value<double>() ??
    infoJson["songApproximativeDuration"]?.Value<double>() ??
    0;

var beatmaps = DiscoverBeatmaps(info, infoJson);
if (beatmaps.Count == 0) throw new Exception("Could not find any beatmap .dat files declared by info.dat");

var models = LoadModels(FindResourcesResx());

Console.WriteLine($"BPM: {bpm}");
Console.WriteLine("Difficulty\tMelchior\tNytilde\tPKBeam\tTimeline");
foreach (var beatmap in beatmaps) {
    var notes = ParseNotes(beatmap.Path);
    ...
    Console.WriteLine($"{beatmap.Name}\t{mel:0.00}\t{nyt:0.00}\t{pk:0.00}\t{tl:0.00}");
}

static List<Note> ParseNotes(string path) {
    var json = JObject.Parse(File.ReadAllText(path));
    var notes = (json["_notes"] ?? json["notes"]) as JArray;
    if (notes != null) {
        return notes
            .Select(n => new Note(
                n["_time"]?.Value<double>() ?? n["time"]?.Value<double>() ?? 0,
                n["_lineIndex"]?.Value<int>() ?? n["lineIndex"]?.Value<int>() ?? 0))
            .OrderBy(n => n.Beat)
            .ThenBy(n => n.Col)
            .ToList();
    }

    var colorNotes = (json["colorNotes"] ?? json["_colorNotes"]) as JArray ?? new JArray();
    return colorNotes
        .Select(n => new Note(
            n["b"]?.Value<double>() ?? n["_time"]?.Value<double>() ?? n["time"]?.Value<double>() ?? 0,
            n["x"]?.Value<int>() ?? n["_lineIndex"]?.Value<int>() ?? n["lineIndex"]?.Value<int>() ?? 0))
        .OrderBy(n => n.Beat)
        .ThenBy(n => n.Col)
        .ToList();
}

static List<BeatmapFile> DiscoverBeatmaps(string infoPath, JObject infoJson) {
    var infoDirectory = Path.GetDirectoryName(infoPath)!;
    var result = new List<BeatmapFile>();
    var sets = (infoJson["_difficultyBeatmapSets"] ?? infoJson["difficultyBeatmapSets"]) as JArray;

    if (sets != null) {
        foreach (var set in sets) {
            var characteristic = set["_beatmapCharacteristicName"]?.Value<string>()
                ?? set["beatmapCharacteristicName"]?.Value<string>()
                ?? set["characteristic"]?.Value<string>()
                ?? "Standard";
            var difficultyBeatmaps = (set["_difficultyBeatmaps"] ?? set["difficultyBeatmaps"]) as JArray;
            if (difficultyBeatmaps == null) continue;

            foreach (var map in difficultyBeatmaps) {
                var filename = map["_beatmapFilename"]?.Value<string>() ?? map["beatmapFilename"]?.Value<string>();
                if (string.IsNullOrWhiteSpace(filename)) continue;

                var path = ResolveCaseInsensitive(infoDirectory, filename);
                if (path == null) continue;

                var difficulty = map["_difficulty"]?.Value<string>() ?? map["difficulty"]?.Value<string>() ?? Path.GetFileNameWithoutExtension(path);
                result.Add(new BeatmapFile($"{difficulty}{characteristic}", path));
            }
        }
    }
    ...
    if (result.Count > 0) {
        return result
            .GroupBy(b => Path.GetFullPath(b.Path), StringComparer.OrdinalIgnoreCase)
            .Select(g => g.First())
            .OrderBy(b => b.Name, StringComparer.OrdinalIgnoreCase)
            .ToList();
    }

    return Directory.GetFiles(infoDirectory, "*.dat", SearchOption.TopDirectoryOnly)
        .Where(p => !string.Equals(Path.GetFileName(p), "info.dat", StringComparison.OrdinalIgnoreCase))
        .OrderBy(p => p, StringComparer.OrdinalIgnoreCase)
        .Select(p => new BeatmapFile(Path.GetFileNameWithoutExtension(p), p))
        .ToList();
}
...
static string FindResourcesResx() {
    var candidates = new[] { Directory.GetCurrentDirectory(), AppContext.BaseDirectory };
    foreach (var candidate in candidates) {
        var directory = new DirectoryInfo(candidate);
        while (directory != null) {
            var path = Path.Combine(directory.FullName, "Properties", "Resources.resx");
            if (File.Exists(path)) return path;
            directory = directory.Parent;
        }
    }

    throw new FileNotFoundException("Could not find Properties/Resources.resx");
}

static (byte[] nytilde, byte[] nytildeFallback, byte[] pkbeamPmml) LoadModels(string resxPath) {
    var doc = XDocument.Load(resxPath);
    byte[] Get(string name) {
        var data = doc.Root!.Elements("data").First(e => string.Equals(e.Attribute("name")?.Value, name, StringComparison.OrdinalIgnoreCase));
        var val = data.Element("value")!.Value.Trim();
        var fileRefPath = val.Split(';')[0];
        if (fileRefPath.EndsWith(".onnx", StringComparison.OrdinalIgnoreCase) ||
            fileRefPath.EndsWith(".pmml", StringComparison.OrdinalIgnoreCase)) {
            var resourcePath = ResolveRelativeCaseInsensitive(Path.GetDirectoryName(resxPath)!, fileRefPath)
                ?? throw new FileNotFoundException($"Could not resolve resource file '{fileRefPath}' from '{resxPath}'");
            return File.ReadAllBytes(resourcePath);
        }

        return Convert.FromBase64String(val);
    }
    return (Get("Edda-MLDP_Nytilde"), Get("Edda-MLDP_Nytilde_Fallback"), Get("Edda-MLDP_PKBeam"));
}

static string? ResolveRelativeCaseInsensitive(string baseDirectory, string relativePath) {
    var current = baseDirectory;
    var parts = relativePath.Replace('\\', Path.DirectorySeparatorChar).Replace('/', Path.DirectorySeparatorChar)
...
static double PKBeam(List<Note> notes,double bpm,double songDuration,byte[] pmml){ var duration=notes.Count>0?60/bpm*notes.Last().Beat:songDuration; if(duration<=0)return 0; var nd=notes.Count/duration; var local=new List<double>(); var low=0.0; var high=2.75; do {var c=notes.Count(n=>{var t=60/bpm*n.Beat; return low<=t&&t<=high;}); local.Add(c/2.75); low+=0.25; high+=0.25;} while(high<duration); var peak=Quantile(local,0.95); var path=Path.GetTempFileName(); File.WriteAllBytes(path,pmml); using var reader=File.OpenText(path); var doc=new PMMLDocument(reader); var curr=CultureInfo.CurrentCulture; CultureInfo.CurrentCulture=CultureInfo.InvariantCulture; try{ using var svm=new SupportVectorMachineModelEvaluator(doc); var res=svm.GetResult(new { BPM=bpm, NoteDensity=nd, HighNoteDensity2s=peak},null); var xml=new XmlDocument(); xml.Load(path); var constants=xml.GetElementsByTagName("Constant"); if(constants.Count<2)throw new InvalidOperationException("PKBeam PMML is missing expected unscale constants"); var unvar=double.Parse(constants[0]!.InnerText,CultureInfo.InvariantCulture); var unmean=double.Parse(constants[1]!.InnerText,CultureInfo.InvariantCulture); return Convert.ToDouble(res.PredictedValue,CultureInfo.InvariantCulture)*unvar+unmean; } finally {CultureInfo.CurrentCulture=curr;} }
...
record BeatmapFile(string Name, string Path);
